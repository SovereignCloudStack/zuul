variable "zuul_vm_name" {
  type        = string
  description = "Name of the Compute Instance for zuul. Default: zuul"
  default     = "zuul"
}

variable "zuul_cloudsyaml_identifier" {
  type        = string
  description = "Identitifier of the OPenStack Project to deploy the resources to"
  default     = "gx-scs_zuul"
}

variable "zuul_network_name" {
  type        = string
  description = "The name of the network that is created for zuul deployment"
  nullable    = false
  default     = "zuul_network"
}

variable "zuul_subnet_cidr" {
  type        = string
  description = "Defined CIDR for zuul"
  nullable    = false
  default     = "192.168.199.0/24"
}

variable "router_id" {
  type        = string
  description = "router the zuul network should be attached to"
  nullable    = false
  default     = "96d4e5a0-4424-4dd5-8dac-ad8231b634f1"
}

variable "zuul_security_group" {
  type        = string
  description = "name of the security group that is about to be created"
  nullable    = false
  default     = "zuul_secgroup"
}

variable "zuul_instance_image_id" {
  type        = string
  description = "image id used for the zuul instance"
  nullable    = false
  default     = "a4a2cddf-6577-4e0c-b8e1-70ac584927df"
}

variable "zuul_flavor_name" {
  type        = string
  description = "flavor name for the zuul instance"
  nullable    = false
  default     = "SCS-8V-32"
}

variable "zuul_inital_ssh_pubkey" {
  type        = string
  description = "key name for the zuul instance"
  nullable    = false
  default     = "ssh-rsa  @o-otte"
}

## dev overwrites
variable "zuul_cloudsyaml_identifier_dev" {
  type        = string
  description = "Identitifier of the OPenStack Project to deploy the resources to"
  default     = "gx-scs_gonicus"
}

variable "zuul_flavor_name_dev" {
  type        = string
  description = "flavor name for the zuul instance"
  nullable    = false
  default     = "SCS-4V-16"
}

# Router ID in Gonicus Project
variable "router_id_dev" {
  type        = string
  description = "router the zuul network should be attached to"
  nullable    = false
  default     = "0ec38c02-c4dc-4d62-ad26-1fed240fed2f"
}
